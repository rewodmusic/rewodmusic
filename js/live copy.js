(() => {
  ...
  // ---- NEW: sync datebox slide with accordion open/close ----
  function getPanelDurationMs(panel) {
    const cs = getComputedStyle(panel);
    // prefer max-height transition if present; otherwise use first duration
    ...
    return Number.isFinite(toMs(durStr)) ? toMs(durStr) : 0;
  }

  function setDateboxShift(card, open) {
    const datebox = card.querySelector(".live-datebox");
    const panel = card.querySelector("[data-panel]");
    if (!datebox || !panel) return;

    // make sure datebox can animate
    ...
    // target shift = expanded panel height / 2
    // (mert a card lefelé nő, a teljes card közepe kb ennyivel megy le)
  ...
  function renderEventCard(item, dateObj) {
      ...
      <article class="live-card" data-card>
        <div class="live-card-inner">
          ...
          <div class="live-btncol">
            <button class="live-btn live-btn-info" type="button" data-action="toggle">
              INFO
            </button>
            <a class="live-btn live-btn-tickets" href="${escapeHTML(ticketsUrl)}" target="_blank" rel="noopener">
              TICKETS
            </a>
          </div>
        </div>
  ...
  function bindInteractions() {
    app.addEventListener("click", (e) => {
      ...
      // NEW: datebox slide synced
      setDateboxShift(card, willOpen);

      // when opening, recalc height next frame
      if (willOpen) {
        requestAnimationFrame(() => {
          panel.style.maxHeight = panel.scrollHeight + "px";
          // and update target shift after height is final
          setDateboxShift(card, true);
        });
      }
    });
  ...
  async function init() {
    try {
      ...
      for (const [, arr] of groups.entries()) {
        ...
        for (const x of arr) html += renderEventCard(x.it, x.d);
      }
      ...
      // reset dateboxes
      const cards = app.querySelectorAll("[data-card]");
      cards.forEach((c) => setDateboxShift(c, false));
    } catch (err) {
      console.error(err);
      renderEmpty();
    }
